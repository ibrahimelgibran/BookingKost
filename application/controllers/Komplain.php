...
class Komplain extends CI_Controller
{
	...
	function tambah($id_hunian){
		...
		// Periksa apakah session 'email_member' memiliki nilai yang valid
		$email = $this->session->userdata('email_member');
		if (!$email) {
			// Handle ketika session email_member tidak memiliki nilai
			// Misalnya, redirect ke halaman login atau tampilkan pesan kesalahan
			redirect('login');
		}
		...
		if (!$member) {
			// Handle ketika data member tidak ditemukan
